...
import com.tayota.operationservice.mapper.appointment.AppointmentCreatedMapper;
import com.tayota.operationservice.mapper.appointment.MyAppointmentDetailMapper;
...
public class AppointmentService {
    ...
    @Transactional
    public AppointmentCreatedResponse createUserTestDriveAppointment(CreateTestDriveAppointmentRequest request, String clientIp) {
        return createTestDriveAppointment(request, getCurrentUserId(), clientIp);
    }
    ...
    @Transactional
    public AppointmentCreatedResponse createUserServiceAppointment(CreateServiceAppointmentRequest request, String clientIp) {
    ...
    @Transactional(readOnly = true)
    public List<AppointmentCreatedResponse> getMyAppointments() {
    ...
    @Transactional(readOnly = true)
    public MyAppointmentDetailResponse getMyAppointmentDetail(UUID appointmentId) {
        UUID userId = getCurrentUserId();
    ...
    private void checkLimit(String key, String message) {
        if (systemCacheService.hasKey(key)) {
    ...
    private void checkDailyLimit(String key, int limit) {
        Object value = systemCacheService.get(key);
    ...
    // Hàm đánh dấu thời gian chờ giữa các lần đặt lịch (cooldown) bằng cách lưu một khóa trong cache hệ thống với TTL tương ứng.
    ...
    private void markLimit(String key, Duration ttl) {
        systemCacheService.put(key, "1", ttl);
    }
    ...
    private void markDaily(String key) {
        Long count = systemCacheService.increment(key);
        ...
        if (count != null && count == 1) {
            systemCacheService.expire(key, Duration.ofDays(1));
        }
    }
